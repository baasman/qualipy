...
def barchart_top_categories_altair(data, column, top_n=20):
    ...
    chart = alt.Chart(d).mark_bar().encode(x=alt.X('category:N', sort='-y'), y=alt.Y(f'{column}:Q')).properties(title='All Categories', width=600)
    chart.display()
...
def value_count_chart_altair(data, var, anom_data, top_n=20):
    ...
    unique_vals = reduce(lambda x, y: x.union(y), [set(i.keys()) for i in data_values])
    df = pd.DataFrame({cat: [i.get(cat, 0) for i in data_values] for cat in unique_vals})
    top_columns = df.sum().sort_values(ascending=False).head(min(len(unique_vals), top_n))
    ...
    base = alt.Chart(df).properties(title=f'{var} - {metric}: Proportion over time', width=600)
    area = base.mark_area().encode(x=alt.X('date:T'), y=alt.Y('value:Q', stack='normalize'), color='variable:N')
    anom_line = alt.Chart(df).mark_rule(color='red').encode(x='date:T', y=alt.Y('anom:Q'))
...
def proportion_change_altair(data, var, top_n=20):
    data_values = [(pd.Series(c, dtype='int') / pd.Series(c, dtype='int').sum()).to_dict() for c in data["value"]]
    traces = []
    unique_vals = reduce(lambda x, y: x.union(y), [set(i.keys()) for i in data_values])
    diffs = []
    cats = []
    ...
    top_cats = df.groupby('category')['proportional change'].sum().head(min(len(unique_vals), top_n))
    df = df[df.category.isin(top_cats.index)]
    
    base = alt.Chart(df).properties(title='Change in Proportion', width=600)
    lines = base.mark_line().encode(x=alt.X('date:T'), y=alt.Y('proportional change:Q'), color='category:N')
    return lines